...
pub trait FsmExt<I: Signal>: Sized {
    ...
    /// Maps the output.
    fn map<O: Signal, F: 'static + Fn(Expr<I>) -> Expr<O>>(self, k: &mut CompositeModuleContext, f: F) -> Self::Out<O> {
        self.fsm_map::<(), O, _>(k, Some("map"), ().into(), move |input, state| (f(input), state))
    }

    /// Feeds `self` to a new `N`-windowing FSM.
    fn window<const N: usize>(self, default: Expr<I>, k: &mut CompositeModuleContext) -> Self::Out<Array<I, U<N>>> {
